...
export const registerUser = async (req, res) => {
  try {
    ...
    // Only set phone if provided (prevents null/empty from violating unique indexes)
    ...
    if (err.code === 11000) {
      const dupMsg = err?.keyValue
        ? `Duplicate value: ${JSON.stringify(err.keyValue)}`
        : "Duplicate value";
      return res.status(400).json({
        success: false,
        message: `Duplicate entry. ${dupMsg}`,
      });
    }
...
export const googleAuthCallback = async (req, res, next) => {
  ...
  passport.authenticate(
    ...
    async (err, user, info) => {
      try {
        console.log("[Google OAuth Callback] Authentication result:", {
          err: err?.message,
        ...
        if (req.query.state) {
          ...
          if (decodedState) {
            try {
              ...
              console.log(
                "[Google OAuth Callback] Parsed state data:",
                stateData,
              );
            } catch {
        ...
        if (stateData && stateData.nonce && stateData.stateId) {
          ...
          console.log(
            "[Google OAuth Callback] Validating state, storedState exists:",
            !!storedState,
          );

          if (
            storedState &&
            storedState.nonce === stateData.nonce &&
            Date.now() - storedState.timestamp < STATE_TTL
          ) {
            ...
            console.log(
              "[Google OAuth Callback] State validated successfully, redirecting to:",
              redirect,
            );
          } else {
            ...
            return res.redirect(
              `${process.env.CLIENT_URL}/login?error=${encodeURIComponent("Invalid or expired authentication state")}`,
            );
          }
        }
        ...
        if (err || !user) {
          console.log(
            "[Google OAuth Callback] Authentication failed, err:",
            err?.message,
            "user:",
            user,
          );
          return res.redirect(
            `${redirect}?error=${encodeURIComponent("Google authentication failed")}`,
          );
        }
        ...
        if (user.status !== "Active") {
          console.log(
            "[Google OAuth Callback] User account not active, status:",
            user.status,
          );
          return res.redirect(
            `${redirect}?error=${encodeURIComponent("Account is not active")}`,
          );
        }
        ...
        const sessionId = crypto.randomBytes(32).toString("hex");
        console.log(
          "[Google OAuth Callback] Generating JWT token for user:",
          user.email,
        );
        ...
        console.log(
          "[Google OAuth Callback] Redirecting to login with auth payload for client session hydration",
        );
  ...
  try {
    ...
    try {
      ...
      if (process.env.NODE_ENV !== "production") {
        return res
          .status(200)
          .json({ success: true, message: "OTP sent (dev mode)", devOtp: otp });
      }
    }
